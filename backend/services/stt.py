...
def transcribe_audio(audio_bytes: bytes, filename: str = "audio.webm", language_hint: Optional[str] = None) -> str:
    """
    ...
    if not (settings.GROQ_API_KEY or settings.GROQ_API_KEY.strip()):
        raise STTError("Speech-to-text is not configured. Missing GROQ_API_KEY.", code="stt_not_configured")
    ...
    # Ensure we have a supported extension for the API
    ext = "." + filename.rsplit(".", 1)[-1].lower() if "." in filename else ""
    if ext not in ALLOWED_EXTENSIONS:
        # Groq accepts common formats; use webm as default if unknown
        filename = "audio.webm" if not filename else filename
    ...
    if resp.status_code >= 500:
        raise STTError("Speech service is temporarily unavailable. Please try again later.", code="stt_server_error")
    ...
    try:
        ...
        if not text:
            raise STTError("No speech detected. Try speaking clearly or check your microphone.", code="stt_no_speech")
        return text
    except STTError:
        raise
    except Exception as e:
