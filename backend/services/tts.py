...
# Language code -> Google TTS languageCode (API picks default voice)
LANG_TO_CODE = {
    "en": "en-IN",
    "hi": "hi-IN",
    "mr": "mr-IN",
...
def synthesize_speech(text: str, lang: str = "en") -> bytes:
    ...
    if not (settings.GOOGLE_TTS_API_KEY or settings.GOOGLE_TTS_API_KEY.strip()):
    ...
    if resp.status_code == 403:
        try:
            ...
            if "API has not been used" in err_msg or "enabled" in err_msg.lower():
                raise TTSError(
                    "Text-to-Speech API is not enabled. Enable it at: console.cloud.google.com/apis/library/texttospeech.googleapis.com",
                    code="tts_unauthorized",
                )
            if "billing" in err_msg.lower() or "quota" in err_msg.lower():
                raise TTSError(
                    "Billing or quota issue. Enable billing for the project (free tier still applies) or check quota.",
                    code="tts_unauthorized",
                )
            raise TTSError(
                f"Access denied (403): {err_msg[:180]}",
                code="tts_unauthorized",
            )
        except TTSError:
            raise
